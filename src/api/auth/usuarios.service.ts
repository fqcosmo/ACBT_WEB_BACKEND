...
import { LoginAccess, ResponseCreateUser, ResponseUpdateUser, ResponsLogin, Usuarios, UsuariosWithRole } from '../model/UsuariosDTO';
...
export class UsuariosService {
    ...
    async updateUser(user: UsuariosWithRole) {
        ...
        if(user.password){
            ...
            const isPasswordValid = bcrypt.compareSync(user.password, users.password);

            if (!isPasswordValid) {
                throw new UnauthorizedException('Contraseña incorrecta');
            }

            const passwordHashed = (await bcrypt.hash(user.newPass, 10)).toString();
            ...
            const responseContraseña : ResponseUpdateUser = {
                message: 'CONTASEÑA ACTUALIZADA',
                status: 200
            }
